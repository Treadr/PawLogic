...
class ABCLog(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "behavior_severity BETWEEN 1 AND 5", name="ck_abc_logs_severity"
        ),
    ...
    antecedent_tags: Mapped[list[str]] = mapped_column(
    ...
    behavior_tags: Mapped[list[str]] = mapped_column(
        ARRAY(String(50)), nullable=False
    )
    ...
    consequence_tags: Mapped[list[str]] = mapped_column(
        ARRAY(String(50)), nullable=False
    )
    ...
    other_pets_present: Mapped[list[uuid.UUID] | None] = mapped_column(
